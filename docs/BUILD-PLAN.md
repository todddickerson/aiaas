...
- **Day 11 / QA + observability shim (2026-05-21)** — Sentry shim at `lib/observability/sentry.ts` exposes `captureException` / `captureMessage` / `isEnabled` so application code can wire into observability without taking on the @sentry/nextjs bundle weight yet (no-op + `console.error` today; swap to the real SDK once the project is provisioned). New `/api/health` endpoint returns `{ ok, uptimeMs, latencyMs, agentCount, sentry, commit }` — this is what the Cloudflare / external monitor pings. QA load smoke (`tests/e2e/qa-loadtest.spec.ts`): three Playwright cases verifying the brief validator handles 50 concurrent calls under 8s p95 with 100% success, the health endpoint returns the expected shape, and every public marketing + agent + manager page returns 200 with a `<title>` element. Plus 4 Vitest cases on the Sentry shim. (PR #17)
- **Day 12 / alpha gate + status page (2026-05-21)** — Per Todd's deployment posture: NO DNS cutover; the app stays on a Vercel subdomain through alpha. New `middleware.ts` reads `AIAAS_ALPHA_TOKEN` and gates every route except `/api/health`, `/api/v1/*`, `/gate`, sitemap/robots/favicon, and Next's static assets. Visitors hit `/gate` (iOS-zoom-safe 16px password input, no emoji, semantic theme tokens) and the gate redirects through `?alpha=<token>` which the middleware turns into a 30-day httpOnly cookie. With `AIAAS_ALPHA_TOKEN` unset the gate is dormant — local dev + the existing 67 Playwright tests keep passing. New `/status` page reads `/api/health` and surfaces agent count + probe latency + uptime + Sentry mode + build SHA — that's the public status page external monitors and Todd both look at. Five new Playwright cases: gate is off by default, `/gate` renders, `/api/health` stays open, `/status` reflects health state, gate form action returns to the `next` path. (PR #18)

- **Drift detection / weekly Lighthouse gate (2026-09-04)** — Closes the plan's "weekly automated Lighthouse run, fails the build if any metric drops >10% week-over-week" line. `lib/monitor/lighthouse.ts` is the framework-free core: 9-page public catalog (marketing + agent + manager + `/status`), `extractScores` (LHR → 0–100 per Perf / A11y / Best-practices / SEO), `compareCategory` (FAIL on a >10% relative WoW drop, WARN under the Day-11 "≥90" floor, PASS otherwise — no-baseline pages can only pass/warn), baseline build/parse, markdown renderer. `scripts/lighthouse-weekly.mjs` is the zero-app-dependency CLI (Node 22 type stripping; shells out to `npx lighthouse@12`, desktop preset, optional alpha-gate cookie via `LIGHTHOUSE_ALPHA_TOKEN`; exit 0 ok / 1 regression / 2 bad URL). `.github/workflows/lighthouse-weekly.yml` runs Mondays 13:00 UTC + `workflow_dispatch`, restores the `lighthouse-baseline` artifact from the last *successful* run (so a persisting regression keeps alerting against the last good week instead of quietly re-baselining), uploads the new baseline + full LHRs, and opens/comments/closes a single `lighthouse-regression` issue exactly like the synthetic monitor. 19 Vitest cases. Seed run against the live alpha: 9/9 pages audited (~15s each), Perf 100 / Best-practices 100 everywhere; A11y 93–96 (every page trips `color-contrast`, home also `definition-list`); SEO 63 on every page because the `.vercel.app` alias serves `x-robots-tag: noindex` (`is-crawlable` fails) — expected pre-cutover, surfaces as WARN not FAIL. Follow-ups: fix the theme's low-contrast text token(s) to lift A11y ≥ 96 → 100; SEO clears itself at the aiaas.com cutover.
