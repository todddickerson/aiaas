...
- **Post-alpha / synthetic production monitor (2026-09-04)** — Closes the "Drift detection in production" line that the 12-day plan left unbuilt. `lib/monitor/synthetic.ts` is the framework-free core: a 6-probe catalog (homepage, `/api/health`, `/status`, `/agents/funnelsmith`, `/managers/todd`, `/sitemap.xml`) plus pure `evaluateProbe` / `summarize` / `renderMarkdown` and an injectable-fetch `runSyntheticMonitor`. Gated pages accept either the real page marker or the `/gate` interstitial, so flipping `AIAAS_ALPHA_TOKEN` on doesn't page anyone. `scripts/synthetic-monitor.mjs` is the zero-dependency CLI (Node 22 type stripping, no `pnpm install`; exit 0 healthy / 1 regression / 2 misconfigured) and writes the table to `$GITHUB_STEP_SUMMARY`. `.github/workflows/synthetic-monitor.yml` runs it every 15 min against `vars.MONITOR_BASE_URL` (defaults to the alpha `aiaas-todddickerson.vercel.app`; the eventual aiaas.com cutover is a repo-variable change), opens/comments a single `synthetic-monitor`-labelled issue on regression and auto-closes it on recovery — Sentry is still the shim, so the issue is the alert channel. 13 Vitest cases. Verified live: all 6 probes green against the alpha deployment (commit 75023e4). Heartbeat audit the same day confirmed the alpha is deployed but production carries **no app env vars** (stub mode for Supabase/Anthropic/Whop/Composio, gate dormant) — that wire-up is Todd's call, not a code slice. (PR #20)
